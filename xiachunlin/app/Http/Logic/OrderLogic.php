    ...
    public function getList($where = array(), $order = '', $field = '*', $offset = '', $limit = '')
    {
        ...
        if($res['count'] > 0)
        {
            foreach($res['list'] as $k=>$v)
            {
                ...
                $res['list'][$k]->province_name = model('Admin\\Region')->getRegionName(['id'=>$v->province]);
                $res['list'][$k]->city_name = model('Admin\\Region')->getRegionName(['id'=>$v->city]);
                $res['list'][$k]->district_name = model('Admin\\Region')->getRegionName(['id'=>$v->district]);
                
                $order_goods = model('Admin\\OrderGoods')->getDb()->where(array('order_id'=>$v->id))->get();
...
//        dd($where);
        ...
        $res->province_name = model('Admin\\Region')->getRegionName(['id'=>$res->province]);
        $res->city_name = model('Admin\\Region')->getRegionName(['id'=>$res->city]);
        $res->district_name = model('Admin\\Region')->getRegionName(['id'=>$res->district]);
        
        $order_goods = model('Admin\\OrderGoods')->getDb()->where(array('order_id'=>$res->id))->get();
    ...
    public function add($data = array(), $type=0)
    {
        ...
        //获取收货地址
        $user_address = model('Admin\\UserAddress')->getOne(['user_id'=>$data['user_id'],'id'=>$data['default_address_id']]);
        ...
        if ($order_id)
        {
			...
			//改变优惠券使用状态
            model('Admin\\UserBonus')->getDb()->where(array('user_id'=>$data['user_id'],'id'=>$data['user_bonus_id']))->update(array('status'=>1,'used_time'=>time()));
			//扣除用户积分，预留
            ...
            foreach($order_goods['list'] as $k=>$v)
            {
                ...
                //订单商品直接减库存操作
				model('Admin\\Good')->changeGoodsStock(array('goods_id'=>$v->goods_id,'goods_number'=>$v->goods_number));
            }
            $result = model('Admin\\OrderGoods')->add($order_goods_list,1);
            if($result)
            {
                //删除购物对应的记录
                model('Admin\\Cart')->getDb()->where(array('user_id'=>$data['user_id']))->whereIn('id', explode("_",$data['cartids']))->delete();
