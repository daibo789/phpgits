...
use App\Model\Admin\User_money;
...
class UserMoneyLogic extends BaseLogic
{
    ...
    public function add($data = array(), $type=0)
    {
        ...
        $user = model('Admin\\User')->getOne(['id'=>$data['user_id']]);
        ...
        if($data['type'] == User_money::USER_MONEY_INCREMENT)
        {
            //增加用户余额
            model('Admin\\User')->getDb()->where(array('id'=>$data['user_id']))->increment('money', $data['money']);
        }
        elseif($data['type'] == User_money::USER_MONEY_DECREMENT)
        {
            //判断用户余额是否足够
            if($data['money']>$user->money){return ReturnData::create(ReturnData::FAIL,null,'余额不足');}
            
            //减少用户余额
            model('Admin\\User')->getDb()->where(array('id'=>$data['user_id']))->decrement('money', $data['money']);
        }
        else
        {
            DB::rollBack(); //事务回滚
        ...
        $user_money = model('Admin\\User')->getDb()->where(array('id'=>$data['user_id']))->value('money'); //用户余额
