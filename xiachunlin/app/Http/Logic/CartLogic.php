...
class CartLogic extends BaseLogic
{
    ...
    public function add($data = array(), $type=0)
    {
        ...
        //获取商品信息
        $goods = model('Admin\\Good')->getDb()->where(['id' => $data['goods_id'], 'status' => Good::GOODS_NORMAL_STATUS])->first();
    ...
    public function cartCheckoutGoodsList($where)
    {
        ...
        if($cartList)
        {
            ...
            {
                $goods = Good::where(array('id'=>$v['goods_id']))->first();
                
                $cartList[$k]->is_promote = 0;
                if(model('Admin\\Good')->bargain_price($goods->price,$goods->promote_start_date,$goods->promote_end_date) > 0){$cartList[$k]->is_promote = 1;}
                
                $cartList[$k]->final_price = model('Admin\\Good')->get_goods_final_price($goods); //商品最终价格
