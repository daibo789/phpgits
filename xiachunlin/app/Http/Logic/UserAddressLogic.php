...
class UserAddressLogic extends BaseLogic
{
    ...
    public function add($data = array(), $type=0)
    {
        ...
        if(model('Admin\\UserAddress')->getDb()->where('user_id', $data['user_id'])->count() >= 10)
        {
        ...
        if($res)
        {
            $user_address = model('Admin\\User')->getDb()
                    ->join('user_addresses', 'users.address_id', '=', 'user_addresses.id')
    ...
    {
        $data->country_name  = isset($data->country) ? model('Admin\\Region')->getRegionName(['id'=>$data->country]) : '';
        $data->province_name = isset($data->province) ? model('Admin\\Region')->getRegionName(['id'=>$data->province]) : '';
        $data->city_name     = isset($data->city) ? model('Admin\\Region')->getRegionName(['id'=>$data->city]) : '';
        $data->district_name = isset($data->district) ? model('Admin\\Region')->getRegionName(['id'=>$data->district]) : '';
