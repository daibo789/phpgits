...
class IndexController extends CommonController
{
    ...
    public function category($cat, $page=0)
    {
        ...
        if(cache("catid$cat")){$post = cache("catid$cat");}else{

            $post = object_to_array(DB::table('arctypes')->where('id', 1)->first(), 1);
//            dd($post);
            if(empty($post)){
        ...
        $data['post'] = $post;
        $subcat="";$sql="";
        $post2 = object_to_array(DB::table('arctypes')->select('id')->where('pid', $cat)->get());
        ...
        $counts = DB::table("articles")->whereRaw($sql)->count();
...
//        dd($post['template']);
    ...
    public function goods($id)
    {
        ...
        $post = object_to_array(DB::table('goods')->where('id', $id)->first(), 1);if(empty($post)){return redirect()->route('page404');}$post['name'] = DB::table('goods_types')->where('id', $post['typeid'])->value('name');
        if($post)
        {
            $cat = $post['typeid'];
            $post['body'] = ReplaceKeyword($post['body']);
        ...
        $posType = object_to_array(DB::table('goods_types')->where('id', $cat)->first(), 1);
//        dd($post);
        return view('wap.index.'.$posType['temparticle'], $data);
    }
