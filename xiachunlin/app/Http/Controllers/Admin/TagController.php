...
class TagController extends CommonController
{
    ...
    {
        $data['posts'] = parent::pageList('tagindices');
    ...
    public function doadd()
    {
        ...
        if($insertId = DB::table('tagindices')->insertGetId($_POST))
        {
            if($tagarc!="")
            {
                $arr=explode(",",$tagarc);
                ...
                foreach($arr as $row)
                {
                    $data2['tid'] = $insertId;
                    $data2['aid'] = $row;
                    DB::table("taglists")->insert($data2);
                }
            }
    ...
    public function edit()
    {
        ...
        $data['id'] = $id;
        $data['post'] = object_to_array(DB::table('tagindices')->where('id',$id)->first(), 1);

        //获取该标签下的文章id
        $posts = object_to_array(DB::table('taglists')->select('aid')->where('tid', $id)->get());
        $aidlist = "";
        if(!empty($posts))
        {
    ...
    public function doedit()
    {
        ...
        if(DB::table('tagindices')->where('id', $id)->update($_POST))
        {
            //获取该标签下的文章id
            $posts = object_to_array(DB::table("taglists")->select('aid')->where('tid', $id)->get());
            $aidlist = "";
            ...
            elseif($tagarc=="")
            {
                DB::table("taglists")->where('tid', $id)->delete();
            }
    ...
    public function del()
    {
        ...
        if(DB::table("tagindices")->whereIn("id", explode(',', $id))->delete())
        {
            success_jump('删除成功');
        }
        else
