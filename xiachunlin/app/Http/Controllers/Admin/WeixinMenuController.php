...
{
    protected $weixinMenuLogic;
    public function __construct(WeixinMenuLogic $weixinMenuLogic)
    {
        parent::__construct();
        $this->weixinMenuLogic = $weixinMenuLogic;
    }
    ...
    {
        $where['is_show'] = 0;
        $catlist = $this->weixinMenuLogic->getPaginate($where, array('id', 'desc'));
    ...
    public function add()
    {
        if(!empty($_GET["reid"]))
        {
            ...
            if($id!=0)
            {
                $data['postone'] = object_to_array(DB::table("weixin_menus")->where('id', $id)->first(), 1);
            }
    ...
    public function doadd()
    {
        ...
        if(DB::table('weixin_menus')->insert($_POST))
        {
            success_jump('添加成功');
        }
        else
    ...
    public function edit()
    {
        ...
        $data['id'] = $id;
        $post = object_to_array(DB::table('weixin_menus')->where('id', $id)->first(), 1);
        $reid = $post['pid'];
        if($reid!=0){$data['postone'] = object_to_array(DB::table('weixin_menus')->where('id', $reid)->first());}
    ...
    public function doedit()
    {
        ...
        if(DB::table('weixin_menus')->where('id', $id)->update($_POST))
        {
    ...
    public function del()
    {
        ...
        if(DB::table('weixin_menus')->where('pid', $id)->first())
        {
            error_jump('删除失败！请先删除子栏目');
        }
        else
        {
            if(DB::table('weixin_menus')->where('id', $id)->delete())
            {
                success_jump('删除成功');
            }
            else
    ...
    public function createmenu(WeixinMenu $weixinMenu)
    {
