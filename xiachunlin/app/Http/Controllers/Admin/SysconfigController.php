...
class SysconfigController extends CommonController
{
    ...
    {
		$data['posts'] = parent::pageList('sysconfigs', '', ['id', 'desc']);
    ...
    public function doadd()
    {
        ...
        {
			if(!preg_match("/^CMS_[a-z]+[_][a-z]+$/i", $_POST["varname"]))
			{
		...
		unset($_POST["_token"]);
		if($_POST['varname']!="" && DB::table('sysconfigs')->insert($_POST))
        {
    ...
    public function edit()
    {
		...
		$data['id'] = $id;
		$data['post'] = object_to_array(DB::table('sysconfigs')->where('id', $id)->first(), 1);
    ...
    public function doedit()
    {
		...
		unset($_POST["_token"]);
		if(DB::table('sysconfigs')->where('id', $id)->update($_POST))
        {
            cache()->forget('sysconfigs'); //删除缓存
    ...
    public function del()
    {
		...
		if(DB::table("sysconfigs")->whereIn("id", explode(',', $id))->delete())
        {
            success_jump('删除成功');
        }
		else
