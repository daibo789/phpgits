...
class GoodsBrandController extends CommonController
{
    ...
    {
        $data['posts'] = object_to_array(DB::table("goods_brands")->select('id', 'add_time', 'title', 'litpic', 'status', 'listorder', 'cover_img', 'click')->orderBy('id', 'desc')->get());
    ...
    public function doadd()
    {
        ...
        if(DB::table("goods_brands")->insert($_POST))
        {
    ...
    public function edit()
    {
        ...
        $data['id'] = $id;
        $data['post'] = object_to_array(DB::table('goods_brands')->where('id', $id)->first(), 1);
    ...
    public function doedit()
    {
        ...
        if(DB::table('goods_brands')->where('id', $id)->update($_POST))
        {
    ...
    public function del()
    {
        ...
        if(DB::table('goods_brands')->whereIn("id", explode(',', $id))->delete())
        {
            success_jump('删除成功');
        }
        else
