...
use App\Model\Admin\OrderGoods;
...
class OrderController extends CommonController
{
    ...
    public function index()
    {
        ...
        $posts = parent::pageList('orders', $where);
        foreach($posts as $key=>$value)
        {
            $order_status_arr = model('Admin\\Order')->getOrderStatusAttr($value);
            ...
            $posts[$key]->province_name = model('Admin\\Region')->getRegionName(['id'=>$value->province]);
            $posts[$key]->city_name = model('Admin\\Region')->getRegionName(['id'=>$value->city]);
            $posts[$key]->district_name = model('Admin\\Region')->getRegionName(['id'=>$value->district]);
        }
    ...
    public function detail()
    {
        ...
        if($data['post'])
        {
            $order_status_arr = model('Admin\\Order')->getOrderStatusAttr($data['post']);
            ...
            $data['post']['province_name'] = model('Admin\\Region')->getRegionName(['id'=>$data['post']->province]);
            $data['post']['city_name'] = model('Admin\\Region')->getRegionName(['id'=>$data['post']->city]);
            $data['post']['district_name'] = model('Admin\\Region')->getRegionName(['id'=>$data['post']->district]);

            $data['post']['invoice_text'] = model('Admin\\Order')->getInvoiceAttr($data['post']);
            $data['post']['place_type_text'] = model('Admin\\Order')->getPlaceTypeAttr($data['post']);
            ...
            {
                $order_goods[$k]['refund_status_text'] = model('Admin\\OrderGoods')->getRefundStatusAttr($v);
            }

            $data['post']['goodslist'] = $order_goods;
        }
        //echo '<pre>';print_r($data['post']);exit;
        $data['kuaidi'] = DB::table('kuaidis')->where(['status'=>0])->orderBy('listorder', 'asc')->get();
    ...
    public function changeShipping(Request $request)
    {
        ...
        else
        {
            ...
            $data['shipping_name'] = DB::table('kuaidis')->where('id', $data['shipping_id'])->value('name');
        }

        if(DB::table('orders')->where(['id'=>$id,'shipping_status'=>0])->update($data) === false)
        {
    ...
    public function changeStatus(Request $request)
    {
        ...
        elseif($status==8)
        {
            ...
            if($order->pay_money>0)
            {
                //返余额
                //增加用户余额
                DB::table('users')->where(array('id'=>$order->user_id))->increment('money', $order->pay_money);
                //添加用户余额记录
                DB::table('user_moneys')->insert(array('user_id'=>$order->user_id,'type'=>0,'money'=>$order->pay_money,'des'=>'退货-返余额','user_money'=>DB::table('users')->where(array('id'=>$order->user_id))->value('money'),'add_time'=>time()));
            }
        ...
        if(DB::table('orders')->where('id', $id)->update($data) === false)
        {
            return ReturnData::create(ReturnData::SYSTEM_FAIL);
        }
    ...
    public function outputExcel(Request $request)
    {
        ...
        $where = function ($query) use ($res) {
            ...
            $query->where('is_delete', 0); //未删除s
        };

        $cellData = [];
        array_push($cellData,['ID','订单号','时间','状态','商品总价','应付金额','支付金额','收货人','地址','电话','订单来源']);
        $order_list = DB::table('orders')->where($where)->orderBy('id', 'desc')->get();
        ...
        if($order_list)
        {
            ...
            {
                $order = new Order();
                $order_status_arr = $order->getOrderStatusAttr($v);
//                $order_status_arr = (new Order)->getOrderStatusAttr();
                ...
                $order_list[$k]->province_name = $region->getRegionName(array('id'=>$v->province));
                $order_list[$k]->city_name = $region->getRegionName(array('id'=>$v->province));
                $order_list[$k]->district_name = $region->getRegionName(array('id'=>$v->province));
//                $order_list[$k]->place_type_text = $region->getPlaceTypeText(['place_type'=>$v->place_type]);

                array_push($cellData,[$v->id,$v->order_sn,date('Y-m-d H:i:s',$v->add_time),$order_list[$k]->order_status_text,$v->goods_amount,$v->order_amount,$v->pay_money,$v->name,$order_list[$k]->province_name.$order_list[$k]->city_name.$order_list[$k]->district_name.' '.$v->address,$v->mobile]);
            }
        }
//        dd($cellData);
        //导出EXCEL
