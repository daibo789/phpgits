...
class GoodsTypeController extends CommonController
{
    ...
    {
        $catlist = category_tree(get_category('goods_types',0));

        if($catlist)
        {
            foreach($catlist as $k=>$v)
            {
                $arctype = DB::table("goods_types")->where('id', $v['id'])->first();
    ...
    public function add()
    {
        if(!empty($_GET["reid"]))
        {
            ...
            if($id!=0)
            {
                $data['postone'] = object_to_array(DB::table("goods_types")->where('id', $id)->first(), 1);
            }
    ...
    public function doadd()
    {
        ...
        if(DB::table("goods_types")->insert($_POST))
        {
    ...
    public function edit()
    {
        ...
        $data['id'] = $id;
        $post = object_to_array(DB::table("goods_types")->where('id', $id)->first(), 1);
        $reid = $post['pid'];
        if($reid!=0){$data['postone'] = object_to_array(DB::table("goods_types")->where('id', $reid)->first(), 1);}
        $data['post'] = $post;
    ...
    public function doedit()
    {
        ...
        if(DB::table("goods_types")->where('id', $id)->update($_POST))
        {
    ...
    public function del()
    {
        ...
        if(DB::table("goods_types")->where('pid', $id)->first())
        {
            error_jump('删除失败！请先删除子分类');
        }
        else
        {
            if(DB::table("goods_types")->where('id', $id)->delete())
            {
