...
class Order extends BaseModel
{
    ...
    {
        $order_goods = model('Admin\\OrderGoods')->getDb()->where(array('order_id'=>$order_id))->get();
        if(!$order_goods){return false;}

        foreach($order_goods as $k=>$v)
        {
            //订单商品直接返库存
            model('\'Admin\\Good')->changeGoodsStock(array('goods_id'=>$v->goods_id,'goods_number'=>$v->goods_number,'type'=>1));
        }
