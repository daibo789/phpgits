...
<title><?php if(isset($post)){echo $post["name"].'_'.sysconfig('CMS_WEBNAME');}else{echo '动态';} ?>
...
<?php
    if($list){
        foreach($list as $k=>$v){ ?>
    <div class="list">
        <?php if(!empty($v['litpic'])){ ?>
            {{--,'catid'=>$v['typeid']--}}
            <a class="limg" href="{{route('shop_articleDetail',array('id'=>$v['id']))}}">
                <img alt="<?php echo $v['title']; ?>" src="<?php echo $v['litpic']; ?>">
            </a><?php } ?>
    <strong class="tit">
        <a href="{{route('shop_articleDetail',array('id'=>$v['id']))}}">
        <?php echo $v['title']; ?></a>
    </strong><p><?php echo mb_strcut($v['description'],0,150,'UTF-8'); ?>..</p>
