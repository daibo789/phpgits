...
<script type="text/javascript" src="/weixins/js/jquery-form.js"></script>
...
function update_nickname()
{
    ...
    layer.open({
        ...
        ,yes: function(index){
            ...
            {
                $.post('<?php echo http_host(true).'/api/user_info_update'; ?>',{nickname:nickname,access_token:'<?php echo $user_info_token['access_token']; ?>'},function(res)
                {
                    if(res.code==0)
                    {
                        //提示
...
{
    $.post('<?php echo http_host(true).'/api/user_info_update'; ?>',{sex:sex,access_token:'<?php echo $user_info_token['access_token']; ?>'},function(res)
    {
        if(res.code==0)
        {
            //提示
...
function qrcode_layer()
{
    //询问框
    layer.open({
        content: '<div><div><img style="width:100%;" class="imgzsy" src="<?php echo get_erweima(route('weixin',array('invite_code'=>$user_info_token['id'])),240); ?>"></div><p style="color:#999;">扫一扫，你懂得</p></div>'
    });
}
</script>
...
function update_refund_account()
{
    ...
    layer.open({
        ...
        ,yes: function(index){
            ...
            {
                $.post('<?php echo http_host(true).'/api/user_info_update'; ?>',{refund_account:refund_account,refund_name:refund_name,access_token:'<?php echo $user_info_token['access_token']; ?>'},function(res)
                {
                    if(res.code==0)
                    {
                        //提示
    ...
    layer.open({
        ...
        ,yes: function(index){
            ...
            else
            {
                ...
                $.post('<?php echo http_host(true).'/api/user_password_update'; ?>',{password:md5(password),old_password:md5(old_password),access_token:'<?php echo $user_info_token['access_token']; ?>'},function(res)
                {
                    if(res.code==0)
                    {
                        //提示
...
function set_pay_password()
{
    ...
    layer.open({
        ...
        ,yes: function(index){
            ...
            {
                $.post('<?php echo http_host(true).'/api/user_password_update'; ?>',{pay_password:md5(pay_password),old_pay_password:'',access_token:'<?php echo $user_info_token['access_token']; ?>'},function(res)
                {
                    if(res.code==0)
                    {
                        //提示
    ...
    layer.open({
        ...
        ,yes: function(index){
            ...
            else
            {
                ...
                $.post('<?php echo http_host(true).'/api/user_password_update'; ?>',{pay_password:md5(pay_password),old_pay_password:md5(old_pay_password),access_token:'<?php echo $user_info_token['access_token']; ?>'},function(res)
                {
                    if(res.code==0)
                    {
                        //提示
