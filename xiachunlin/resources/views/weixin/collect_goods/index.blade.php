...
<div class="floor">
    <ul class="goods_list_s cl">
        ...
        <li><a href="<?php echo route('weixin_goods_detail',array('id'=>$v['goods']['id'])); ?>"><span class="goods_thumb"><img alt="<?php echo $v['goods']['title']; ?>" src="<?php echo http_host(true); ?><?php echo $v['goods']['litpic']; ?>"></span></a>
...
function del(goods_id)
{
    ...
    layer.open({
        ...
        ,btn: ['确定', '取消']
        ,yes: function(){
            var url = '<?php echo http_host(true)."/api/collect_goods_delete"; ?>';
            $.post(url,{goods_id:goods_id,access_token:'<?php echo $weixin_user_info['access_token']; ?>'},function(res)
            {
                //提示
                layer.open({
                    content: res.msg
