...
<script language="javascript" type="text/javascript" src="/vendor/js/layer/layer.js"></script>
...
function fahuo_layer(order_id)
{
    ...
    layer.open({
        ...
        ,yes: function(index, layero){
            ...
            $.post('<?php echo route('admin_order_change_shipping'); ?>',{_token:'{{csrf_token()}}',id:order_id,shipping_id:shipping_id,shipping_sn:shipping_sn },function(res)
            {
                if(res.code==0)
                {
                    $.post('<?php echo route('admin_order_change_status'); ?>',{_token:'{{csrf_token()}}',id:order_id,status:3},function(res)
                    {
                        if(res.code==0)
                        {
                            layer.msg('操作成功');
