...
<title><?php echo sysconfig('CMS_WEBNAME'); ?></title>
    ...
    <link rel="stylesheet" href="waps/css/wap.css" media="all">
    <script type="text/javascript" src="waps/js/jquery.min.js"></script>
</head><body>

<header class="mheader-a">
    <h2 class="bt"><?php echo sysconfig('CMS_WEBNAME'); ?></h2>
    <a href="javascript:;">
        <div class="phone-icon telphone"></div>
    </a>
</header>

@include('wap.common.header')
<article class="mbanner-b mb10"><a href=""><img src="waps/images/banner.jpg"></a></article>
...
<ul class="mnewpic2">
    <?php $posts=arclist(array("table"=>"goods","row"=>4,"typeid"=>1,"expression"=>[["litpic","<>","''"]])); ?>
        ...
        @endforeach
            ...
            <a href="<?php echo get_wap_front_url(array('catid'=>1,'type'=>'productlist')); ?>">查看更多
                <i class="mico"></i>
            </a></div>
</div></article>
...
<div class="about-more more">
    <a href="/wap{{get_wap_front_url(array('pagename'=>'about','type'=>'page'))}}">
        查看更多
        <i class="phone-icon mico"></i>
    </a></div>
</article>

<article class="mbox"><h3 class="tit"><i class="tit_icon"></i>新闻中心</h3>
    <ul class="mnews-b" id="ajlist">
        <?php $posts=arclist(array("row"=>8)); ?>
        @foreach($posts as $row)
                <li><span class="group">[<?php echo date("Y-m-d",$row['pubdate']); ?>]</span><a href="wap{{get_wap_front_url(array("id"=>$row['id'],"catid"=>$row['typeid'],"type"=>'content'))}}"><?php echo $row['title']; ?></a></li>
        @endforeach
    </ul>
    <div class="about-more more mt10">
        <a id="listbu" href="javascript:;">查看更多
            <i class="phone-icon mico"></i>
        </a>
    </div>
</article>
